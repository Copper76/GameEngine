...
//	glm::vec3 p0 = s1 + ab * ab.Dot(ap)/ab.GetLengthSqr(); // project origin onto the line. Huh?
//
//	int idx = 0;
//	float mu_max = 0;
//	for (int i = 0; i < 3; i++) {
...
//	glm::vec3 normal = (s2 - s1).Cross(s3 - s1);
//	glm::vec3 p0 = normal * s1.Dot(normal) / normal.GetLengthSqr();
//
//	int idx = 0;
//	float area_max = 0;
//
...
//	glm::mat4 M;
//	M.rows[0] = glm::vec4(s1.x, s2.x, s3.x, s4.x);
//	M.rows[1] = glm::vec4(s1.y, s2.y, s3.y, s4.y);
//	M.rows[2] = glm::vec4(s1.z, s2.z, s3.z, s4.z);
//	M.rows[3] = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);
//
//	glm::vec4 C4;
//	C4[0] = M.Cofactor(3, 0);
//	C4[1] = M.Cofactor(3, 1);
...
//};
//
//point_t Support(const Body* bodyA, const Body* bodyB, glm::vec3 dir, const float bias) {
...
//	simplexPoints[0] = Support(bodyA, bodyB, glm::vec3(1, 1, 1), 0.0f);
//
...
//================================
//*/
//void GJK_ClosestPoints(const Body* bodyA, const Body* bodyB, glm::vec3& ptOnA, glm::vec3& ptOnB) {
//	const glm::vec3 origin(0.0f);
//
...
//	int numPts = 1;
//	point_t simplexPoints[4];
//	simplexPoints[0] = Support(bodyA, bodyB, glm::vec3(1, 1, 1), bias);
//
...
namespace Fengshui
{
	...
	point_t Support(const Collider colliderA, const Transform transA, const Collider colliderB, const Transform transB, glm::vec3 inDir, const float bias) {
		...
		point.ptA = colliderA.Shape->Support(dir, transA, colliderA, bias);

		dir *= -1.0f;

		point.ptB = colliderB.Shape->Support(dir, transB, colliderB, bias);
