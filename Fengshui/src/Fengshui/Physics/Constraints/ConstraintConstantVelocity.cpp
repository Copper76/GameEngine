...
namespace Fengshui
{
	...
	*/
	void ConstraintConstantVelocityLimited::PreSolve(const float dt_sec)
	{
		...
		m_angleU = 2.0f * asinf(glm::dot(glm::vec3(qrr.x, qrr.y, qrr.z), u)) * 180.0f / pi;//convert radian to angle as well
		m_angleV = 2.0f * asinf(glm::dot(glm::vec3(qrr.x, qrr.y, qrr.z), v)) * 180.0f / pi;

		m_isAngleViolatedU = (m_angleU > 45.0f || m_angleU < -45.0f);
		m_isAngleViolatedV = (m_angleV > 45.0f || m_angleV < -45.0f);
